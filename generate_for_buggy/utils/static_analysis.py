...
def get_type_to_full_name(node , new_class):
    import_map = new_class.import_map
    ...
    if node.type == 'array_type':
        return get_type_to_full_name(node.child_by_field_name('element'), new_class) + node.child_by_field_name('dimensions').text.decode()
    return node.text.decode()
...
def find_method(node , new_class , package , single_file):
    ...
    if node.type in ['constructor_declaration' , 'method_declaration']:
        ...
        if node.type == "method_declaration":
            # the full type name will be updated in later analysis
            ...
            if child.type == 'formal_parameter':
                ...
                for mod_child in child.children:
                    if mod_child.type == 'modifiers':
                        modifier_flag = True
                        parameter_modifiers.append(mod_child.text.decode())
                
                if not modifier_flag:
                    parameter_modifiers.append("no modifier")
        ...
        if flag and import_node is not None:   # 如果 import * 就把这个包的所有class和method加入到single_file.import_map和method_map中
            if not is_static:
                ...
                if package_name in package_map:
                    ...
                    for classs in package.classes:
                        ...
                        for method in classs.methods:
                            ...
                            single_file.import_method_map[(method.name, tuple(method.parameters_list))] = method
                else:
                    # JDK package
                    single_file.import_jdk_map[package_name] = {'all': True , "classes": set()}  # classes: {class_name : method_name}
            else:
                ...
                if package_name in package_map:
                    ...
                    single_file.import_static_jdk_map[class_name] = {'all': True , "item": set()}
            ...
            if not is_static:
                ...
                if package_name in package_map:
                    ...
                    if package_name in single_file.import_jdk_map:
                        if not single_file.import_jdk_map[package_name]['all']:
                            single_file.import_jdk_map[package_name]['classes'].add(class_name)
                    else:
                        single_file.import_jdk_map[package_name] = {'all': False , "classes": set()}
                        single_file.import_jdk_map[package_name]['classes'].add(class_name)

                single_file.import_map[class_name] = import_node.text.decode()  # 完整类名
            elif is_static:
                method_or_field_name_node = import_node.child_by_field_name('name')
                ...
                method_or_field_name = method_or_field_name_node.text.decode()
                ...
                if package_name in package_map:
                    ...
                    for classs in single_package.classes:
                        if classs.name_no_package == class_name:
                            find_flag = False
                            for method in classs.methods:
                                if method.name_no_package == method_or_field_name and method.is_static:
                                    method_map[(method.name, tuple(method.parameters_list))] = method
                                    single_file.import_static_method[(method.name, tuple(method.parameters_list))] = method
                                    find_flag = True
                                    break
                    ...
                    if whole_class_name in single_file.import_static_jdk_map:
                        if not single_file.import_static_jdk_map[whole_class_name]['all']:
                            single_file.import_static_jdk_map[whole_class_name]['item'].add(method_or_field_name)
                    else:
                        single_file.import_static_jdk_map[whole_class_name] = {'all': False , "item": set()}
                        single_file.import_static_jdk_map[whole_class_name]['item'].add(method_or_field_name)
...
def find_class_field(node , classs , single_file):
    if node.type in ['field_declaration', 'constant_declaration']:
        ...
        if type_node is None:
            return
        if type_node.type in ['integral_type', 'floating_point_type']:
            for variable_node in node.children:
                ...
                classs.class_variable_map[variable_name] = type_node.text.decode()
        elif type_node.type in ['scoped_type_identifier', 'type_identifier']:
            for variable_node in node.children:
                if variable_node.type != 'variable_declarator':
                    continue
                variable_name = variable_node.child_by_field_name('name').text.decode()
                ...
                if variable_value_node is None:
                    ...
                    classs.class_variable_map[variable_name] = single_file.import_map[type_name] if type_name in single_file.import_map else type_name
...
def find_class_variable(node , variable_map , classs):
    ...
    if node.type in ['field_declaration', 'constant_declaration']: # String s = getName(a, b);  field_declaration: private int count;
        ...
        if type_node is None:
            return
        if type_node.type in ['integral_type', 'floating_point_type']: # int a = 1;
            for variable_node in node.children: # eg: int x = 1 , y = 2;
                if variable_node.type != 'variable_declarator':
                    continue
                ...
                classs.fields[variable_name] = node.text.decode()
        elif type_node.type in ['scoped_type_identifier', 'type_identifier']: # ClassA a = ...;  ClassA.ClassB b = 
            for variable_node in node.children:  
                if variable_node.type != 'variable_declarator':
                    continue
                variable_name = variable_node.child_by_field_name('name').text.decode()   # variable name
                variable_value_node = variable_node.child_by_field_name('value')   # value
                if variable_value_node is None:  # only declaration: variable_map = variable_name : type_name
                    type_name = type_node.text.decode()
                    variable_map[variable_name] = import_map[type_name] if type_name in import_map else type_name
                    classs.fields[variable_name] = node.text.decode()
                elif variable_value_node.type == 'identifier': # ClassA b = a;
                    decoded_text = variable_value_node.text.decode()
                    if decoded_text in variable_map:  # defined before ?
                        variable_map[variable_name] = variable_map[decoded_text]
                        classs.fields[variable_name] = node.text.decode()
                elif variable_value_node.type == 'object_creation_expression': # ClassA a = new ClassA();
                    ...
                    classs.fields[variable_name] = node.text.decode()
                else: # eg: ClassA a = getClassA();
                    type_name = type_node.text.decode()
                    variable_map[variable_name] = import_map[type_name] if type_name in import_map else type_name
                    classs.fields[variable_name] = node.text.decode()
                    
        elif type_node.type == 'array_type': # int[][] a = new int[5][5];
        ...
        elif type_node.type == 'generic_type': # List<String> list; 
            type_name = None
...
def find_call_method(package , method_map , class_map):
    for classs in package.classes:
        ...
        for method in classs.methods:
            ...
            method_variable_map = copy.deepcopy(variable_map)  # fields
